...
public class TrainerMealPlanRepository {
    private final MealPlanDao mealPlanDao;
    private final Executor executor;

    @Inject
    public TrainerMealPlanRepository(MealPlanDao mealPlanDao, Executor executor) {
    ...
    private void refreshMealPlanList(String token, long userId) {
        ...
        if (!mealPlanExist) {
            ...
            executor.execute(() -> {
                ...
                ApiInterface apiService =
                        ApiClient.getClient().create(ApiInterface.class);
                Call<RetMealPlanList> call = apiService.getTrainerMealPlanList(token, userId, 0, 100);
                try {
                    ...
                    if (response.isSuccessful()) {
                        ...
                        Log.d("refreshMealPlanList", "run: newDao save:" + mealPlanDao.saveList(response.body().getRecords()).length);
