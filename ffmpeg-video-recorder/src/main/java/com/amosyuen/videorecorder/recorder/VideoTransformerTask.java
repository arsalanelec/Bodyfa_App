...
public class VideoTransformerTask implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            while (!frameGrabbers.isEmpty()) {
                ...
                Log.d("FFmpegFrameFilter", "run: outputSize:"+outputSize);
                if(wrapper.getFilterParams()==null){
                    Log.d("FFmpegFrameFilter", "run: wrapper.getFilterParams():null");
                }
               /* FFmpegFrameFilter filter =
                        getFilter(filterMap, wrapper.getFilterParams(), outputSize);*/
                Log.d("FFmpegFrameFilter", "still working 1");
    ...
    protected FFmpegFrameFilter getFilter(
        ...
        Log.d("filter", "getFilter: 1:"+TextUtils.join(",", transforms)
                +"\n2:"+imageSize.getWidthUnchecked()
                +"\n3:"+imageSize.getHeightUnchecked()
                +"\n4:"+params.getFrameRate()
                );
